...
class SupabaseCycleRepository {
  ...
  Future<CycleEntry?> getForDate(DateTime date) async {
    ...
    try {
      ...
      print('Erro ao obter entrada de ciclo: $e');
      return null;
    }
  }
  ...
  Future<void> save(CycleEntry entry) async {
    ...
    try {
      ...
      final data = {
        'entry_date': dateStr,
        'symptoms': entry.symptoms.map((s) => s.toString().split('.').last).join(','),
        'menstrual_flow': entry.menstrualFlow?.toString().split('.').last,
      ...
      print('Erro ao guardar entrada de ciclo: $e');
      rethrow;
    }
  }
  ...
  Future<DateTime?> getLastPeriodStart() async {
    ...
    try {
      ...
      for (int i = 0; i < entries.length; i++) {
        ...
        if (entry.menstrualFlow != null && entry.menstrualFlow != MenstrualFlow.none) {
          // Verificar se é início (dia anterior sem fluxo ou não existe)
          if (i == entries.length - 1) {
            return entry.entryDate;
          }
          final prevEntry = entries[i + 1];
          final daysDiff = entry.entryDate.difference(prevEntry.entryDate).inDays;
          if (daysDiff > 1 || prevEntry.menstrualFlow == null || prevEntry.menstrualFlow == MenstrualFlow.none) {
            return entry.entryDate;
          }
        }
      }
